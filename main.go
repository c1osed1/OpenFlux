...
func main() {
	...
	// OpenFlux fork extras (c1osed1): extra exit/client options on top of upstream.
...
`)
	...
	case "oneme":
		...
		inner = oneme.NewOneMeTransport(*role == roleExit, maxToken, callees, config, *callDelay, icePayload)
	case "cupsonline":
