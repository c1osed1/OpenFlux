...
Форк [p1neappleXpress/OpenFlux](https://github.com/p1neappleXpress/OpenFlux) с рабочими патчами (`--bind-ip`, `--max-payload`, `--channel`, reconnect MAX-приёмника, `--call-delay`). См. [CHANGELOG.md](CHANGELOG.md).
...
| `--mode` | `-m` | `l3` | Режим выходной ноды: `l3` \| `l4` (алиасы `raw`=`l3`, `proxy`=`l4`) |
...
| `--maxUid` | | | ID пользователя MAX (`--transport=oneme`), через запятую — failover |
| `--bench-bytes` | | `0` | Сколько MB залить (`--role=bench-send`) |
| `--bench-compressible` | | `false` | Сжимаемый payload (bench) |

Дополнительные флаги форка:

| Флаг | По умолчанию | Описание |
|------|--------------|----------|
| `--bind-ip` | пусто | Исходящий IPv4 exit-ноды (несколько адресов, proxy/l4) |
