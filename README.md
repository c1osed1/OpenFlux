...
Fork of [p1neappleXpress/OpenFlux](https://github.com/p1neappleXpress/OpenFlux) with operational patches (`--bind-ip`, `--max-payload`, `--channel`, MAX reconnect). See [CHANGELOG.md](CHANGELOG.md).
...
| `--mode` | `-m` | `l3` | Exit-node mode: `l3` \| `l4` (`raw`=`l3`, `proxy`=`l4` aliases) |
...
| `--maxUid` | | | MAX user id (`--transport=oneme`), comma-separated for failover |
...
|------|---------|-------------|
| `--bind-ip` | empty | Exit-node source IPv4 (multi-IP hosts, proxy/l4 mode) |
| `--call-delay` | `3` | Seconds to wait before MAX outgoing call |
| `--max-payload` | `ice` | MAX path: `ice` (signaling) or `dc` (WebRTC DataChannel) |
| `--channel` | empty | Yandex cursor channel so two tunnels can share one doc |
